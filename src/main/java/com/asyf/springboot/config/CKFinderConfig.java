...
public class CKFinderConfig extends Configuration {
    ...
    @Override
    protected Configuration createConfigurationInstance(HttpServletRequest request) {
        //在此更改文件路径
        //this.servletConf;
        System.err.println(this.baseDir);
        System.err.println(this.baseURL);
        HttpSession session = request.getSession();
        User user = (User) session.getAttribute("user");
        System.err.println(user.toString());
        String id = user.getName();
        this.baseDir = "D:/upload/" + id + "/";
        this.baseURL = "/upload/" + id + "/";
        //cloneConfiguration方法中执行copyConfFields方法，将本类的属性复制到super.createConfigurationInstance()
        // 新new的confid
        return super.createConfigurationInstance(request);
    }
